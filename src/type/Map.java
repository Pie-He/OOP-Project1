...
public class Map {
	...
	public void event(Player player, int dice) {
		...
		if (!(places[player.getPoi()] instanceof Bank))
	...
	private Place getRealInstance(JSONObject jo) {
		...
		return null;
		/*
		 * switch (symbol) { case "◎": return JSON.toJavaObject(jo,
		 * House.class); case "券": return JSON.toJavaObject(jo, Coupon.class);
		 * case "道": return JSON.toJavaObject(jo, Shop.class); case "新": return
		 * JSON.toJavaObject(jo, News.class); case "银": return
		 * JSON.toJavaObject(jo, Bank.class); case "彩": return
		 * JSON.toJavaObject(jo, Lottery.class); case "空": return
		 * JSON.toJavaObject(jo, Space.class); case "卡": return
